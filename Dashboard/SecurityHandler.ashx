e<%@ WebHandler Language="C#" Class="SecurityHandler" %>
...
public class SecurityHandler : IHttpHandler, System.Web.SessionState.IReadOnlySessionState
{
    ...
    {
                string cpath = context.Session["ContentPath"].ToString();
        string path=string.Empty;
        string free = context.Session["free"].ToString();
        var Id = context.Request.QueryString["param1"].ToString(); 
        var fname=context.Request.QueryString["param2"].ToString(); 
            var res=context.Request.QueryString["param3"].ToString(); 
            var ext=context.Request.QueryString["param4"].ToString(); 

        if (free == "free")
            {
            path = System.Web.HttpContext.Current.Server.MapPath("../EduResource/Free/" +Id+"/"+fname+"/"+res);
            }
        if (free == "demo")
            {
            path = System.Web.HttpContext.Current.Server.MapPath("../EduResource/Demo/" +Id + "_Demo"+"/"+fname+"/"+res);
            }
        if(free=="paid")
            {
