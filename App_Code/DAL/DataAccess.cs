...
public class DataAccess
{
    ...
    public static System.Collections.Generic.List<Employees> GetAllForum(int CommentID)
    {
        System.Collections.Generic.List<Employees> listEmployee = new System.Collections.Generic.List<Employees>();
        SqlConnection conn = new SqlConnection();
        conn.ConnectionString = ConfigurationManager.AppSettings["EpathshalaCon"];
        using (SqlConnection con = new SqlConnection(conn.ConnectionString))
        {
            //SqlCommand cmd = new SqlCommand("select StudentTeacherForumDetail.UserID,StudentTeacherForumDetail.QuestionAnswer,StudentTeacherForumDetail.CreatedDate from StudentTeacherForumDetail where ForumID=@ParentCommentID", con);
            SqlCommand cmd = new SqlCommand("selectStudentTeacherForumDetails", con);
            cmd.CommandType = CommandType.StoredProcedure;
            //cmd.CommandText = "selectStudentTeacherForumDetails";

            cmd.Parameters.AddWithValue("@RoleID", AppSessions.RoleID);
            cmd.Parameters.AddWithValue("@ForumID", CommentID);
            //SqlParameter parameter = new SqlParameter();
            //parameter.ParameterName = "@ParentCommentID";
            ...
            while (rdr.Read())
            {
                ...
                employee.UserName = rdr["FirstName"].ToString();
                string date = rdr["CreatedDate"].ToString();
                DateTime dt = Convert.ToDateTime(rdr["CreatedDate"].ToString());
                date = dt.ToString("dd MMMM yyyy");
                //date = date.Substring(0, date.LastIndexOf("/") + 5);
                employee.CommentDate = date;
                employee.commentMessage = rdr["QuestionAnswer"].ToString();
